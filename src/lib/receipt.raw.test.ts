import { describe, expect, it } from 'vitest'
import { parseReceipt } from './receipt'

const ALLO_PIZZA = `ALLO PIZZA DI NAPOLI
26 RUE DE BOISSY
91480 QUINCY SOUS SENART
Tel. 01 69 00 20 20
Caisse-01: DINAPOLI .D
16/01/2025 20:44
Ticket 00167150 16/01/2025
*% À EMPORTER 035 ++
1 MENU PROMO | 25.00
NET A PAYER TTC EURO 25.00
RESTE A REGLER 25.00
Tva Mnt-tva Base...HT Base..TTC
110%) 2.27 22.73 25.00
Nb articles:1 Nb Lig:1 Nb imp:1
Oscar Ver 14.1.Ac-ESign:B000FK6e
Siret 810664094000 10<APE :56 10C>
Tva:FR49810664094-20250116 a 20h44
MERCI POUR VOTRE COMMANDE - A BIENTOT
----------------------------------------
ALLO PIZZA DI NAPOLI
26 RUE DE BOISSY
91480 QUINCY SOUS SENART
Tel. 01 69 00 20 20
Caisse-01: DINAPOLI .D
16/01/2025 20:44
Ticket 00167150 16/01/2025
*% À EMPORTER 035 ++
1 MENU PROMO | 25.00
NET A PAYER TTC EURO 25.00
RESTE A REGLER 25.00
Tva Mnt-tva Base...HT Base..TTC
110%) 2.27 22.73 25.00
Nb articles:1 Nb Lig:1 Nb imp:1
Oscar Ver 14.1.Ac-ESign:B000FK6e
Siret 810664094000 10<APE :56 10C>
Tva:FR49810664094-20250116 a 20h44
MERCI POUR VOTRE COMMANDE - A BIENTOT`

const DAR_ZAMANE = `DAR, ZAMANE
53100 — HOMIREUIL - FRANCE
Tol © 09.63.09.23,38
SIRE:9007 1885600011 NAF:5610R - Na" 10R:FRAT900 18956
JUSTIFICATIF NON VALABLE POUR ENCATSSEMENT
samedi 04 janvier 2025 23:28:17 1123731
Ticket 13537
Operateur : Admin
2 x REPAS
TOTAL HI 59.09 EUR
1 TVA 10% 59.09 5.41
Montant par personne : 32.50 EUR
REGLEMENTS
04/01/2025 CB 65.00 EUR
Top Caisse 3.5.4 (NF625) B.0068
Signature : dveC
Merci de votre visite.`

const FREE_INVOICE = `Site Internet : mobile.free.fr
Service abonné au 3244 (appel gratuit depuis une ligne Free)
7€ e Horaires sur mobile.free.fr/contact html
Forfait Free 5G EISI CONSULTING
N° de ligne : 0602731193 IMED - EISI CONSULTING MANSOUR
Identifiant : 47055602 6 RUE DES 2 COMMUNES
91480 QUINCY SOUS SENART
e Facture n° 2293306919 du 29 janvier 2025
Total de la facture HT 16.66 €
TVA [20.00%] 3.33 €
Somme à payer TTC* 19.99 €
"Cette somme sera prélevée sur votre compte le 30-01-2025.`

const PARKING = `0
Î PARKING

SANS CONTACT
East

LE 16-01-25 À 19-42-27
ORY P4A CB CA

SéonLY

4093112

5201662800053 [
30004

F00000 0000047416
e32aab859da4ad7e

108 001 008080 000414
Ho AUTO:

MONTANT 11,50 EUR
DEBIT

TICKET CLIENT

A CONSERVER

(he ia (20,007)
AL jorasdfotsoss —

'Accueil parking 01 48622107`

const ALPHA_PARKING = `| 4 PARKING
de
maniSatroront pm
PARKING
PARKING
1,92 EUR
T0R(20,00%)
11,50 EUR CB EN428 |
120 :53463
16/01/25 19:42:15pppcpéa
Peis:
16/01/25 18:33: > mes]
— — po
'Accueil parking 01 48622107`

const RESTAURATION_PASSION = `RESTAURATION DE PASSION
2 Bis AVENUE DE QUINCY
77380 COMBS LA VILLE
TEL:01.60.62.80.54 / 06.74..75.35.47
NOTE N° 6134
TICKET N° 41141
Mardi 14 Janvier 2025
STANDARD (SUR PLACE)
Serveur : cEnaNT
2 fepas complets) 34,00 €
PAYÉ 34,00 «
Tw wm R
Ta 10 sn am um
Tora we am wa
TT te Veen wn vero
de votre visite
Siret : 832 914 642 00018
Code WF: 56106`

const VAL_DE_SENART = `LE VAL DE SENART
LA Fenn deux meats
Adi ue sous-Tanget
xxx
S1 28/01/2025 14:07:09 NS35205-1513

52405294100019/FR04524052941/5610A
Cases BETNEREL FE Dies esse
2 Repas Complet(s)

TOTAL TTC: 42.00 Eur AG. Zur
Justificatif non valable pour encaissement
SmartCaisse V 4.6.0 B0065 9akD
| Type TVA HT | ma | me |
10%: 2486 | 2.45 20.00
| Ac 1422 | 078 1005
[ Tot | 8740 | 460 42.00
----------------------------------
LE VAL DE SENART
10 rue des deux communes
91480 Quincy-sous-Senart
* kk
S1 28/01/2025 14:07:09 NS35205-1513

52405294100019/FR04524052941/5610A
Caisse 1:GENERAL (1): 1 impression
2 Repas Complet(s)

TOTAL TTC: 42.00 Eur 42.00 Eur
Justificatif non valable pour encaissement
SmartCaisse V 4.6.0 B0065 9akD
| Type TVA HT | wa | TC |
| 10%: 2455 | 245 27.00
| TVA 5,5% 1422 078 15.00
| Total | 3710 | 490 | 4200`

describe('parseReceipt — samples bruts de 01_TODO_01', () => {
  it('ALLO PIZZA DI NAPOLI (brut, doublon)', () => {
    const r = parseReceipt(ALLO_PIZZA, 'pizza.png')
    expect(r.enseigne).toBe('ALLO PIZZA DI NAPOLI')
    expect(r.date).toBe('2025-01-16')
    expect(r.montantTTC).toBe(25)
    expect(r.montantHT).toBe(22.73)
    expect(r.category).toBe('Restaurant')
  })

  it('DAR, ZAMANE (brut)', () => {
    const r = parseReceipt(DAR_ZAMANE)
    expect(r.enseigne).toBe('DAR, ZAMANE')
    expect(r.date).toBe('2025-01-04')
    expect(r.montantTTC).toBe(65)
    expect(r.montantHT).toBe(59.09)
    expect(r.category).toBe('Restaurant')
  })

  it('Facture Free (brut)', () => {
    const r = parseReceipt(FREE_INVOICE)
    expect(r.enseigne).toContain('Forfait Free 5G EISI CONSULTING')
    expect(r.date).toBe('2025-01-29')
    expect(r.montantTTC).toBe(19.99)
    expect(r.montantHT).toBe(16.66)
    expect(r.category).toBe('Téléphone')
  })

  it('PARKING (brut)', () => {
    const r = parseReceipt(PARKING)
    expect(r.enseigne).toContain('PARKING')
    expect(r.date).toBe('2025-01-16')
    expect(r.montantTTC).toBe(11.5)
    expect(r.montantHT).toBe(9.58)
    expect(r.category).toBe('Stationnement')
  })

  it('ALPHA PARKING (second ticket, brut OCR)', () => {
    const r = parseReceipt(ALPHA_PARKING)
    expect(r.enseigne).toContain('PARKING')
    expect(r.date).toBe('2025-01-16')
    expect(r.montantTTC).toBe(11.5)
    expect(r.montantHT).toBe(9.58)
    expect(r.category).toBe('Stationnement')
  })

  it('RESTAURATION DE PASSION (brut)', () => {
    const r = parseReceipt(RESTAURATION_PASSION)
    expect(r.enseigne).toBe('RESTAURATION DE PASSION')
    expect(r.date).toBe('2025-01-14')
    expect(r.montantTTC).toBe(34)
    expect(r.category).toBe('Restaurant')
  })

  it('LE VAL DE SENART (brut, doublon)', () => {
    const r = parseReceipt(VAL_DE_SENART)
    expect(r.enseigne).toBe('LE VAL DE SENART')
    expect(r.date).toBe('2025-01-28')
    expect(r.montantTTC).toBe(42)
    expect(r.montantHT).toBe(37.1)
    expect(r.category).toBe('Restaurant')
  })
})